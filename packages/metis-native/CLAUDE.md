...
### The codec tier: a known tone through four real encoders

`audio-codecs.test.ts` covers FLAC, Vorbis, AAC and ALAC using
`tests/assets/tone-*.{flac,ogg,m4a}`, generated by
`generate-audio-fixtures.ts` (needs ffmpeg; outputs committed, so the suite does
not) and totalling ~143 KB.

**Synthetic content, real encoders — and that split is the point.** The tier-2
property that matters for *codec* coverage is that libFLAC, libvorbis and
ffmpeg's AAC/ALAC encoders were not written to make these tests pass. The
content being a known tone is what buys assertions the MP3 cannot support: each
channel is checked for the tone it should carry *and* the tone it should not,
which is the only way a channel swap is observable.

Demonstrated by mutation rather than assumed: swapping stereo channels in
`decode_source` fails 7 tests — all four codec cases, the cross-codec pitch
test, and the two WAV channel-layout tests — while **`audio-real-files.test.ts`
passes untouched**, because "the channels differ" is still true when they are
swapped. The two tiers are complementary; neither is redundant.

Two other assertions worth keeping: FLAC and ALAC are lossless, so their decoded
output must be **bit-identical** to the source WAV (any deviation at all fails),
and the two must agree bit-for-bit with *each other* — independent
implementations of the same promise, so if only one matched the reference the
reference would be the suspect.

**AAC is the one deviation, and it is pinned rather than papered over.** Three
numbers disagree: the source is 48000 frames, the MP4 header says 49024, and the
decoder returns 49152. symphonia is asked for gapless decoding and trims MP3,
Vorbis and ALAC to the source length, but not AAC-in-MP4 — so an AAC asset
starts slightly late. Irrelevant for music, potentially audible for a sound
effect triggered on a frame.

**How much delay is deliberately not written down anywhere**, and the reason
generalises: the fixture's tone repeats every 100 samples, so cross-correlation
determines the offset only *modulo 100*. A search returned 1724, which is
1024 mod 100 — consistent with a 1024-sample delay and not evidence of one.
Measuring it needs an aperiodic fixture (an impulse or a sweep). A periodic
test signal is excellent for pitch and useless for latency.
