//! The **uncompressed** loader: decode a pixel image file (PNG/JPEG/TGA/HDR)
//! into a single-mip `GpuTexture`.
//!
//! Row strides here are in **pixels** (`width * bytes_per_pixel`) and the output
//! is always one mip level. The block-compressed rules — strides in blocks, 4x4
//! alignment, mip pyramids — live in [`super::compressed`] and do not apply to
